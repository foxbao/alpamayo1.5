...
补充：64 个 waypoint 是扩散过程**一次性去噪**出来的，不是一个个「写」出来的，
所以没有任何理由让后面的点对前面的点不可见。
...
两个容易说错的地方：
- **是单向的，不是「互相看」**：Expert 里只有动作 token 当 query
  （`k = cat([前缀 cache, 自己的 k])`，而 `q` 只有动作那一份），所以
  **动作能看到前缀，前缀看不到动作**。前缀的 K/V 是 VLM 早就算好并冻结的。
- **动作进的不是 VLM**：它进的是 `self.expert` 自己那一套层（独立权重，
  连 `embed_tokens` 都没有）。从 VLM 那儿借的**只有逐层 K/V**，不共享任何一层权重。

**4-4** 真实用 **②**（实测 `Qwen3VLTextModel` 第 0 层只有 `self_attn + mlp`，没有 cross-attn）。

能不能复用 cache，取决于 **Expert 需要什么当输入**：
- **①** 的 `cross_attn` 要**自己算** `K = W_k·cond`、`V = W_v·cond`（用它自己的
  `W_k/W_v`），所以必须拿到条件的 **hidden 张量**。VLM 缓存里的 K/V 是 VLM 那套
  投影投出来的，对 ① 的 cross_attn 没用。
- **②** 的 block **根本没有 cross_attn**：只算 `Q = W_q·动作`，然后
  `K/V = cat(VLM 的 cache, 自己的)` —— VLM 的缓存**直接当 K/V 用**，不用再过投影。

一句话：**① 要 hidden（它得自己算 K/V），② 要的就是 K/V 本身。**

维度能对上也是前提：Expert 由 `copy.deepcopy(vlm.config.text_config)` 再按
`expert_cfg` 覆盖得到，36 层、`kv_heads=8`、`head_dim=128` 与 VLM 文本塔一致。
...
两个补充：
- ② **没法「把条件设成 null」**——它压根没有 condition 参数，只能从 cache 那一侧动。
  这个「没有入口」本身也是答案的一部分。
- 严格说「全 0 条件」不是「没有条件」，它仍是个合法输入；判据只是「改了会变」。

**4-6** **对等关系**，不是进阶关系——两种连接拓扑，不存在「①学会了才轮到②」。
toy 主线（stage4~12）选 ① 是有理由的：它的条件是**显式张量** `(B,L,H)`，
stage7~12 能直接 `print(condition.shape)` 看清「历史 8 步」「图片 17 个 patch」
是怎么拼起来的；② 的条件埋在 K/V 里，只能看到 `K(1,4,32,64)`，看不出装了什么。
⇒ 别把「教学上方便」误读成「① 更初级」。

**4-7** **② 不需要额外 forward；① 要在生成之后【再补一次完整 forward】。**
原因是 4-4 那条：① 要的是 hidden 张量，而 `generate()` 只给你 token 和 K/V，
所以拿 condition 时必须把 backbone 从头跑一遍；② 要的 K/V 在 `generate()` 时
就顺手攒下来了，**白拿**。
⇒ 这正是真实代码能写一句 `expert(..., past_key_values=prompt_cache)` 的原因。
stage13/14 的 Part 1 vs Part 2 就是这条代价差跑出来的现场。

**4-8** 是**同一种 block**（都是 `CacheBlock`），而且必须**同层数**（这里都是 2 层）。
cache 是「**每一层一份 K/V**」，层数或结构不一致，Expert 拿到的东西就对不上号。
⇒ 真实里对应 GQA：两侧 `kv_heads=8`、`head_dim=128` 对齐，所以
hidden 4096(VLM) 与 2048(Expert) 不同也能共用 cache（见 `real3_kv_cache.py`）。

**4-9** - `causal=True` → **`PrefixVLM`**：前缀是 VLM 自回归生成的，位置 i 只能看 0..i
- `causal=False` → **`PrefixExpert`**：64 个 waypoint 互相可见
一个 block 要两种模式，是因为**两份数据的性质不同**：前缀是「文本式、从左到右」的，
动作是「轨迹式、一个整体」的 —— 后者就是 4-2 那个理由，在 prefix 版里又出现一次。
⇒ 真实里对应 `expert_non_causal_attention=True`。
...
**13-5b** **对等关系**，不是进阶关系——两种连接拓扑，不存在「①学会了才轮到②」。
留着 Part 1 有两个理由：①「prefix 也能把条件传过去」这句话需要**对照组**，
同一个 backbone / 同一份数据 / 同一个任务，唯一变量是连接方式；
② cross-attn 的条件是**显式张量**，stage7~12 靠它才能 `print(condition.shape)` 看清维度。
⚠️ 选 prefix 的理由是【真实代码就那么写的】，**不是②在 toy 上跑分更高**——
单一 seed、这么小的模型不构成效果比较。

**13-6** 靠**因果 mask**：位置 j 的 K/V 只依赖 token 0..j，永远看不到后面的 token，
所以后面追加多少 token 都不影响已算好的 K/V。这就是「能 cache」的全部理由
（`exp_kv_cache.py` 实测两种写法输出完全相同）。

**13-7** Part 1 用 `cond_pad_mask` → cross-attn 的 `key_padding_mask`；
Part 2 用 `cache_pad_mask` → attention 里 `masked_fill(-inf)`。
这是**同一个问题在两个位置**的翻版（条件张量侧 / 前缀 K/V 侧），
两者都不能靠 `IGNORE_INDEX` 解决。
