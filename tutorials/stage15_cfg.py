"""Stage 15: CFG —— 用分类器自由引导放大「左转」指令（独立分支，从 stage6 出发）

【目的】理解 Classifier-Free Guidance 的完整闭环：
  - **训练侧**：以 50% 概率把条件替换成「空条件」，让空条件学【边缘分布】
    （即「左转 ∪ 右转」的混合，而不是某个中间方向）
  - **推理侧**：同一步用同一份 x 跑两次网络，再外推：
        v = (1-w)·v_uncond + w·v_cond
    w=0 → 边缘分布；w=1 → 条件分布；w>1 → 外推向量场，加大条件的影响
  - 观察方式：固定同一份初始噪声，只改 w，看平均曲率 κ 怎么变
    （固定噪声是关键——否则采样随机性会淹没 w 的效果）

⚠️ 三个容易误解的点：
  - **CFG 不等于曲率放大器**：w>1 不保证曲率单调增大，更不保证更安全/更准确
  - **w=0 不是「直行」**：空条件学的是左/右的【边缘分布】，不是零动作
  - 代价是**每步两次前向**，推理成本翻倍（真实 CFG 还要维护两份 KV cache）

【简化】相对 `sample_trajectories_from_data_with_vlm_rollout_cfg_nav`：
  - toy 学了一个**可学习的空条件 embedding** 来表示「无指令」；
    真实代码是**从输入序列里删掉 `<|route_start|>...<|route_end|>` 那一段**
    （`nav_utils.remove_nav_text`），再跑一遍 VLM 得到无条件的 KV cache —— 机制相同，构造方式不同
  - 真实的无条件分支**仍有视觉和历史**，只是相对导航文本「无条件」，
    并非移除所有条件；toy 的空条件是真的什么都没有
  - 条件只有 2 个（左/右）+1 个空条件，没有图/历史/文本/CoC
  - 没有实现真实里「先 prefill 无前缀、再 batch_repeat_interleave 复用 cache」那套工程优化
  - 两条分支共用一份初始噪声；真实实现里两条分支的 cache 布局不同
"""
