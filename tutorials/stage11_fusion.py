"""Stage 11: 多模态融合 —— 历史 + 图片 + 文本 → 一条 condition 序列

【目的】学会「三个模态怎么合成一条序列」，这是 VLA 的工程核心：
  - 三路各自编码成 token：历史 (B,8,64) + 视觉 (B,17,64) + 文本 (B,2,64)
  - 直接在**序列维 concat** 成 (B,27,64)，再统一加位置编码
  - 位置编码是必需的：裸 concat 不携带位置信息，且固定槽位（历史在前、文本在后）
    本身就是一种「模态身份」提示
  - 模型行为：三路输入一起给，输出轨迹
...
【简化】
  - 历史编码器退化成一层 Linear（HistoryEncoder 只有 proj，无 transformer）
  - 文本编码器退化成纯 Embedding（**没有位置编码、没有 transformer**）——
    只有 2 个 token 且顺序固定，所以这个简化在本例里不暴露问题
  - 融合方式是裸 concat + 可学习位置编码；真实 VLM 内部对完整序列做上下文化
    （视觉 token 与文本 token 在 30+ 层里逐层交互）
  - 三路长度固定（8/17/2），没有变长、没有 padding mask
  - 仍是 cross-attention 版 Expert，无 CoC、无多相机
"""
